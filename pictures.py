...
def saveRawPictureFromPath(pictureId: str, picture_path: str, isBlurred: bool):
    """
    Save picture from a file path directly into the appropriate filesystem.
    """
    picInPermanentStorage = isBlurred or current_app.config["API_BLUR_URL"] is None
    fses = current_app.config["FILESYSTEMS"]
    picFs = fses.permanent if picInPermanentStorage else fses.tmp
    ...
    with open(picture_path, "rb") as f:
        picFs.writefile(utils.pictures.getHDPicturePath(pictureId), f)
